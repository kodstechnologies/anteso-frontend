/**
 * Generates Dental CBCT upload template with input columns only.
 * Computed columns (Average, Remarks, CoV, CoL, etc.) are left empty — UI calculates them.
 */
import fs from 'fs';
...
const rows = [
  ['TEST: ACCURACY OF OPERATING POTENTIAL'],
  ['Applied kVp', 'mA 1', 'mA 2', 'Average kVp', 'Remarks'],
  ['60', '60.2', '60.5', '', ''],
  ['70', '70.1', '70.4', '', ''],
  ['80', '80.3', '80.1', '', ''],
  [],
  ['TEST: TOTAL FILTRATION'],
  ...
  [],
  ['TEST: RADIATION LEAKAGE LEVEL FROM X-RAY TUBE HOUSE'],
  ...
  [],
  ['TEST: RADIATION PROTECTION SURVEY REPORT'],
...
const outPath = path.join(templateDir, 'Dental_CBCT_Test_Data_Template.csv');
...
try {
  ...
  if (err.code === 'EBUSY') {
    const altPath = path.join(templateDir, 'Dental_CBCT_Test_Data_Template_input_only.csv');
    fs.writeFileSync(altPath, csvContent, 'utf8');
    console.log(`⚠ Main template locked; wrote ${altPath}`);
    console.log('  Close Excel and re-run: node scripts/generateDentalCBCTTemplate.js');
  } else {
    throw err;
  }
}
