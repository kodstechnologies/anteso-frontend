// components/TestTables/OBI/LinearityOfMaLoading.tsx
import React, { useState, useEffect, useMemo } from 'react';
import { Plus, Trash2, Loader2, Edit3, Save } from 'lucide-react';
import toast from 'react-hot-toast';
import {
  addLinearityOfMaLoadingForOBI,
  getLinearityOfMaLoadingByServiceIdForOBI,
  updateLinearityOfMaLoadingForOBI,
} from "../../../../../../api";
...
const LinearityOfMaLoading: React.FC<Props> = ({ serviceId, testId: propTestId, onRefresh, refreshKey, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      console.log('LinearityOfMaLoading: Loading initial data from CSV:', initialData);
      ...
      setIsEditing(true);
      setIsLoading(false);
      console.log('LinearityOfMaLoading: CSV data loaded into form');
    }
  }, [initialData]);
  ...
  useEffect(() => {
    ...
    const loadTest = async () => {
      setIsLoading(true);
      try {
        const data = await getLinearityOfMaLoadingByServiceIdForOBI(serviceId);
        if (data?.data) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!currentTestId) {
        try {
          const existing = await getLinearityOfMaLoadingByServiceIdForOBI(serviceId);
      ...
      if (currentTestId) {
        result = await updateLinearityOfMaLoadingForOBI(currentTestId, payload);
        toast.success("Updated successfully");
      } else {
        result = await addLinearityOfMaLoadingForOBI(serviceId, payload);
...
//       let result;
//       if (testId) {
//         result = await updateLinearityOfMaLoadingForOBI(testId, payload);
//       } else {
//         result = await addLinearityOfMaLoadingForOBI(serviceId, payload);
//         if (result?.data?._id) {
...
//         <h2 className="text-2xl font-bold">Linearity of mA Loading</h2>
//         <button
