...
export default function TubeHousingLeakage({ serviceId, testId: propTestId, onRefresh, refreshKey, initialData }: Props) {
    ...
    useEffect(() => {
        if (initialData) {
            console.log('TubeHousingLeakage: Loading initial data from CSV:', initialData);
            ...
            if (initialData.leakageMeasurements && initialData.leakageMeasurements.length > 0) {
                setLeakageRows(
                    initialData.leakageMeasurements.map((m) => ({
                        location: m.location || '',
                        left: m.left || '',
                        right: m.right || '',
                        front: m.front || '',
            ...
            setIsEditing(true);
            setIsLoading(false);
            console.log('TubeHousingLeakage: CSV data loaded into form');
        }
    }, [initialData]);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const res = await getTubeHousingLeakageByServiceIdForOBI(serviceId);
                const data = res?.data;
                if (data) {
                    ...
                    if (Array.isArray(data.leakageMeasurements) && data.leakageMeasurements.length > 0) {
                        ...
                        // Ensure Tube is always present even if no data
                        setLeakageRows([{
                            location: 'Tube',
                            left: '',
                            right: '',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (!currentTestId) {
                try {
                    const existing = await getTubeHousingLeakageByServiceIdForOBI(serviceId);
            ...
            if (currentTestId) {
                // Update existing
                result = await updateTubeHousingLeakageForOBI(currentTestId, payload);
                toast.success('Updated successfully!');
            } else {
                // Create new
                result = await addTubeHousingLeakageForOBI(serviceId, payload);
