// src/components/TestTables/OBI/AccuracyOfOperatingPotential.tsx
...
import {
    ...
    updateAccuracyOfOperatingPotentialForOBI,
} from "../../../../../../api";
...
const AccuracyOfOperatingPotential: React.FC<AccuracyOfOperatingPotentialProps> = ({
    ...
    const [isLoading, setIsLoading] = useState(!initialData && !!initialTestId);
    ...
    // Load CSV Initial Data (separate useEffect like Mammography)
    useEffect(() => {
        if (initialData) {
            console.log('AccuracyOfOperatingPotential OBI: Loading initial data from CSV:', initialData);
            console.log('AccuracyOfOperatingPotential OBI: initialData.mAStations:', initialData.mAStations);
            console.log('AccuracyOfOperatingPotential OBI: initialData.measurements:', initialData.measurements);
            console.log('AccuracyOfOperatingPotential OBI: initialData.measurements.length:', initialData.measurements?.length);
            
            setIsLoading(true);
            
            if (initialData.mAStations && initialData.mAStations.length > 0) {
                console.log('Setting mAStations:', initialData.mAStations);
                setMAStations(initialData.mAStations);
            }
            
            if (initialData.tolerance) {
                console.log('Setting tolerance:', initialData.tolerance);
                setToleranceSign(initialData.tolerance.sign as "+" | "-" | "±");
                setToleranceValue(initialData.tolerance.value);
            }
            
            if (initialData.totalFiltration) {
                console.log('Setting totalFiltration:', initialData.totalFiltration);
                setTotalFiltration({
            ...
            if (initialData.measurements && initialData.measurements.length > 0) {
                ...
                const newRows: RowData[] = initialData.measurements.map((m, index) => {
                    ...
                    const hasValidData = !isNaN(rowApplied) && rowApplied > 0;
                ...
                console.log('Setting rows with CSV data, count:', newRows.length);
                setRows(newRows);
            } else {
                console.warn('No measurements found in initialData');
            }
            
            setIsSaved(false);
            setIsLoading(false);
            console.log('AccuracyOfOperatingPotential OBI: CSV data loaded into form');
        }
    }, [initialData]);
    ...
    useEffect(() => {
        const loadTest = async () => {
            ...
            try {
                ...
                if (initialTestId) {
                    // If we have a testId, we could use getById, but for now use serviceId
                    const result = await getAccuracyOfOperatingPotentialByServiceIdForOBI(serviceId);
                    if (result?.data) {
                        data = result.data;
                    }
                } else {
        ...
        loadTest();
    }, [serviceId, initialTestId, refreshKey, initialData]);
    ...
    const saveTest = async () => {
        ...
        try {
            ...
            if (!currentTestId) {
                try {
                    const existing = await getAccuracyOfOperatingPotentialByServiceIdForOBI(serviceId);
            ...
            if (currentTestId) {
                // Update existing
                result = await updateAccuracyOfOperatingPotentialForOBI(currentTestId, payload);
                toast.success("Updated successfully");
            } else {
                // Create new
                result = await addAccuracyOfOperatingPotentialForOBI(serviceId, payload);
