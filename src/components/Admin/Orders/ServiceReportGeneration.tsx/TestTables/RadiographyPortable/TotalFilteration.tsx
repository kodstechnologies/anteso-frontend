// Accuracy Of Operating Potential & Total Filtration for Radiography Portable (same UI as Radiography Fixed)
...
import {
  ...
  getAccuracyOfOperatingPotentialByTestIdForRadiographyPortable,
...
// Map backend tolerance sign to UI
const backendSignToUi = (sign: string): "+" | "-" | "±" => {
  if (sign === "plus") return "+";
  if (sign === "minus") return "-";
  return "±";
};

const uiSignToBackend = (sign: "+" | "-" | "±"): string => {
  if (sign === "+") return "plus";
...
const TotalFilteration: React.FC<TotalFilterationProps> = ({
  ...
  const buildRowMetrics = (
    ...
    const measuredValuesStatus = measuredValues.map((val) => {
      ...
      return checkTolerance(measured, applied, tol, tolSign);
    });
    ...
    if (hasValidData) remarks = hasAnyFailure ? "FAIL" : "PASS";
  ...
  useEffect(() => {
    ...
    const tolVal = String(initialData.tolerance?.value ?? toleranceValue);
      ...
      setRows(
        initialData.measurements.map((m: any, i: number) => {
          ...
          return {
            ...
            appliedKvp,
            measuredValues: vals,
            ...buildRowMetrics(appliedKvp, vals, tolSign, tolVal),
          };
        })
      );
    }
    ...
    if (initialData.filtrationTolerance) {
      setFiltrationTolerance((prev) => ({
        ...prev,
        forKvGreaterThan70: String(initialData.filtrationTolerance.forKvGreaterThan70 ?? prev.forKvGreaterThan70),
        forKvBetween70And100: String(initialData.filtrationTolerance.forKvBetween70And100 ?? prev.forKvBetween70And100),
        forKvGreaterThan100: String(initialData.filtrationTolerance.forKvGreaterThan100 ?? prev.forKvGreaterThan100),
        kvThreshold1: String(initialData.filtrationTolerance.kvThreshold1 ?? prev.kvThreshold1),
        kvThreshold2: String(initialData.filtrationTolerance.kvThreshold2 ?? prev.kvThreshold2),
      }));
    }
  }, [csvDataVersion, initialData]);

  // Load existing test data (Portable AOP API)
  useEffect(() => {
    const loadTest = async () => {
      ...
      try {
        let data: any = null;
        if (initialTestId) {
          const result = await getAccuracyOfOperatingPotentialByTestIdForRadiographyPortable(initialTestId);
          data = result?.data;
        } else {
          const result = await getAccuracyOfOperatingPotentialByServiceIdForRadiographyPortable(serviceId);
          data = result?.data;
        }
          ...
          if (data.measurements && data.measurements.length > 0) {
            setRows(
              data.measurements.map((m: any) => {
                ...
                const sign = loadedToleranceSign;
                ...
                const hasAnyFailure = measuredStatus.some((s: boolean) => s === false) || avgStatus === false;
                const hasValidData =
                  !isNaN(rowApplied) &&
                  rowApplied > 0 &&
                  !isNaN(tol) &&
                ...
                let remark: "PASS" | "FAIL" | "-" = (m.remarks as "PASS" | "FAIL" | "-") || "-";
  ...
  const saveTest = async () => {
    ...
    const payload = {
      ...
      tolerance: {
        sign: uiSignToBackend(toleranceSign),
        type: "kvp",
    ...
    try {
      ...
      if (!currentTestId) {
        try {
          const existing = await getAccuracyOfOperatingPotentialByServiceIdForRadiographyPortable(serviceId);
      ...
      if (currentTestId) {
        await updateAccuracyOfOperatingPotentialForRadiographyPortable(currentTestId, payload);
        toast.success("Updated successfully");
      } else {
        const result = await addAccuracyOfOperatingPotentialForRadiographyPortable(serviceId, payload);
      ...
      prev.map((row) => {
        ...
        if (field === "appliedKvp") {
          ...
          return {
            ...row,
            appliedKvp: value,
            ...buildRowMetrics(value, row.measuredValues, toleranceSign, toleranceValue),
          };
        }
        ...
        return {
          ...
          ...buildRowMetrics(row.appliedKvp, newMeasured, toleranceSign, toleranceValue),
        };
      })
    );
  };
            ...
            onChange={(e) => {
              ...
              setRows((prev) =>
                prev.map((row) => {
                  ...
                  if (hasValidData) remark = hasAnyFailure ? "FAIL" : "PASS";
                  return { ...row, measuredValuesStatus: newMeasuredStatus, averageKvpStatus: avgStatus, remarks: remark };
                })
              );
            }}
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            ...
            <input
              ...
              className={`w-24 px-3 py-2 text-lg font-bold text-center border-2 rounded-lg ${
                isSaved ? "border-gray-300 bg-gray-50 text-gray-500 cursor-not-allowed" : "border-gray-400 focus:border-green-500 focus:ring-4 focus:ring-green-200"
              }`}
              placeholder="80"
            />
        ...
        <ul className="space-y-3 text-amber-800">
          <li className="flex items-center gap-3 flex-wrap">
            ...
            <input
              ...
              onChange={(e) => {
                setFiltrationTolerance({ ...filtrationTolerance, forKvGreaterThan70: e.target.value });
                setIsSaved(false);
              }}
              disabled={isSaved}
            ...
            <input
              ...
              onChange={(e) => {
                setFiltrationTolerance({ ...filtrationTolerance, kvThreshold1: e.target.value });
                setIsSaved(false);
              }}
              disabled={isSaved}
          ...
          <li className="flex items-center gap-3 flex-wrap">
            ...
            <input
              ...
              onChange={(e) => {
                setFiltrationTolerance({ ...filtrationTolerance, forKvBetween70And100: e.target.value });
                setIsSaved(false);
              }}
              disabled={isSaved}
              className={`w-20 px-2 py-1 text-center border rounded font-bold ${isSaved ? "border-gray-300 bg-gray-50 text-gray-500 cursor-not-allowed" : "border-amber-600 text-amber-900 bg-white"}`}
            ...
            <input
              ...
              onChange={(e) => {
                setFiltrationTolerance({ ...filtrationTolerance, kvThreshold2: e.target.value });
                setIsSaved(false);
              }}
              disabled={isSaved}
              className={`w-16 px-2 py-1 text-center border rounded font-bold ${isSaved ? "border-gray-300 bg-gray-50 text-gray-500 cursor-not-allowed" : "border-amber-600 text-amber-900 bg-white"}`}
          ...
          <li className="flex items-center gap-3 flex-wrap">
            ...
            <input
              ...
              onChange={(e) => {
                setFiltrationTolerance({ ...filtrationTolerance, forKvGreaterThan100: e.target.value });
                setIsSaved(false);
              }}
              disabled={isSaved}
              className={`w-20 px-2 py-1 text-center border rounded font-bold ${isSaved ? "border-gray-300 bg-gray-50 text-gray-500 cursor-not-allowed" : "border-amber-600 text-amber-900 bg-white"}`}
      ...
      <div className="flex justify-end mt-6">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition ${
            isSaved ? "bg-orange-600 text-white hover:bg-orange-700" : "bg-teal-600 text-white hover:bg-teal-700"
