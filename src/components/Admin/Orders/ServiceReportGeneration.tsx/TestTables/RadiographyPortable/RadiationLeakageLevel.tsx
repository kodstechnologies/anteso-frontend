...
export default function TubeHousingLeakage({ serviceId, testId: propTestId, onRefresh, initialData, csvDataVersion, csvData, refreshKey }: Props) {
  ...
  // Apply CSV/Excel initial data (same as Fixed)
  useEffect(() => {
  ...
  // Process each row: calculate max, result (mR/h), mGy/h, and per-row remark (same as Fixed)
  ...
  // Load CSV Data (legacy csvData/refreshKey)
  useEffect(() => {
    if (csvData && Array.isArray(csvData) && csvData.length > 0 && !initialData) {
      ...
      if (workData) setWorkload(workData.Value);
      ...
      if (rowIndices.length > 0) {
        setLeakageRows(rowIndices.map((idx) => {
          const r = measDataGrouped[idx];
          return {
            location: r.location || '',
            left: r.left || '', right: r.right || '', front: r.front || '', back: r.back || '', top: r.top || '',
            max: '', result: '', unit: 'mR/h', mgy: '',
          };
        }) as LeakageRow[]);
      }
      const tolData = csvData.find((row: any) => row['Field Name'] === 'Tolerance_Value');
      if (tolData) setToleranceValue(tolData.Value);
      setIsEditing(true);
      setHasSaved(false);
    }
  }, [csvData, refreshKey, initialData]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (currentTestId) {
        await updateRadiationLeakageLevelForRadiographyPortable(currentTestId, payload);
        toast.success('Updated successfully!');
      } else {
        const result = await addRadiationLeakageLevelForRadiographyPortable(serviceId, payload);
  ...
  return (
    <div className="p-6 max-w-full overflow-x-auto space-y-8">
      ...
      {/* Exposure Level Table (same as Fixed) */}
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
      ...
      {/* Work Load and Max Leakage Calculation (same as Fixed) */}
      ...
      {/* Summary of Maximum Radiation Leakage (same as Fixed) */}
      <div className="bg-white shadow-md rounded-lg p-6">
