...
const ViewServiceReportCArm: React.FC = () => {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`CArm-Report-${report?.testReportNumber || "report"}.pdf`);
    } catch (error) {
          ...
          <div className="max-w-5xl mx-auto print:max-w-none">
            ...
            {testData.exposureRateTableTop && (
              <div className="mb-16 print:mb-12 print:break-inside-avoid">
                <h3 className="text-xl font-bold mb-6">1. Exposure Rate at Table Top</h3>
            ...
            {testData.outputConsistency && (
              <div className="mb-16 print:mb-12 print:break-inside-avoid">
                <h3 className="text-xl font-bold mb-6">3. Consistency of Radiation Output</h3>
                    ...
                    <table className="w-full border-2 border-black text-sm">
                      <thead className="bg-gray-100">
                        <tr>
                          <th className="border border-black p-3">kVp</th>
                          <th className="border border-black p-3">mA</th>
                          <th className="border border-black p-3">Resolution (lp/mm)</th>
                          <th className="border border-black p-3">Remarks</th>
                        </tr>
                      </thead>
                      <tbody>
                        {testData.lowContrastResolution.resolutionRows.map((row: any, i: number) => (
                          <tr key={i} className="text-center">
                            <td className="border p-3">{row.kvp || "-"}</td>
                            <td className="border p-3">{row.ma || "-"}</td>
                            <td className="border p-3">{row.resolution || "-"}</td>
                            <td className="border p-3">
                              <span className={row.remark === "Pass" ? "text-green-600" : row.remark === "Fail" ? "text-red-600" : ""}>
                                {row.remark || "-"}
                              </span>
