...
const LowContrastResolutionInventionalRadiology: React.FC<Props> = ({
  ...
  // PASS if measured < standard (same as FixedRadioFluro: smaller is better)
  ...
  useEffect(() => {
    const loadTest = async () => {
      ...
      setIsLoading(true);
      try {
        const res = await getLowContrastResolutionByServiceIdForInventionalRadiology(serviceId, tubeId ?? null);
  ...
  useEffect(() => {
    if (csvData && csvData.length > 0) {
      const smallestVal = csvData.find((r: any) => r['Field Name'] === 'Table1_smallestHoleSize')?.['Value'];
      if (smallestVal) setSmallestHoleSize(String(smallestVal));
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <div className="px-6 py-6 bg-gray-50 border-t">
          <div className="flex items-center justify-center gap-8">
            ...
            <span
              className={`inline-flex px-12 py-5 text-md font-extrabold rounded-full shadow-lg ${
                remark === 'PASS'
          ...
          <div className="mt-6 text-center">
            ...
            {remark && (
              <p className="mt-3 text-lg font-semibold">
                {parseFloat(smallestHoleSize) || smallestHoleSize} mm &lt; {recommendedStandard} mm →{' '}
                <span className={remark === 'PASS' ? 'text-green-600' : 'text-red-600'}>{remark}</span>
              </p>
            )}
          </div>
        </div>
