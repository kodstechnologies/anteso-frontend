...
const AccuracyOfIrradiationTime: React.FC<AccuracyOfIrradiationTimeProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      let result;
      if (testId) {
        result = await updateAccuracyOfIrradiationTime(testId, payload);
      } else {
        result = await createAccuracyOfIrradiationTime(serviceId, payload);
        const newId = result?.data?._id || result?.data?.data?._id || result?._id || result?.data?._id;
        if (newId) {
          setTestId(newId);
        }
      }
