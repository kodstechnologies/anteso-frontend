...
/** Normalize Inventional "Measurement of mA Linearity" payload to a shape compatible with RadiographyFixed linearity logic */
function getLinearityDoc(testData: any): any | null {
  if (testData.linearityOfMasLoading?.table2 && Array.isArray(testData.linearityOfMasLoading.table2)) {
    return testData.linearityOfMasLoading;
  }
  const m =
    testData.measurementOfMaLinearity ||
    testData.MeasurementOfMaLinearityInventionalRadiology ||
    null;
  if (!m) return null;
  const table2 = m.table2 || m.Table2;
  if (!Array.isArray(table2) || table2.length === 0) return null;
  const table1 = m.table1 || m.Table1;
  return {
    table2,
    table1,
    tolerance: m.tolerance ?? m.Tolerance ?? "0.1",
    toleranceOperator: m.toleranceOperator ?? m.ToleranceOperator ?? "<=",
    col: m.col ?? m.Col,
    coefficient: m.coefficient,
    colValue: m.colValue ?? m.ColValue,
    remarks: m.remarks ?? m.Remarks,
    kv: m.kv ?? m.kV,
    setKv: m.setKv ?? m.setKV,
  };
}

/** Inventional legacy: table1 time filled → mAs linearity title; align with Fixed `hasTimer` (true → mA label). */
function inferHasTimerFromLinearityDoc(doc: any): boolean {
  const t1 = doc?.table1;
  const first = Array.isArray(t1) ? t1[0] : t1;
  const timePresent =
    first?.time != null && String(first.time).trim() !== "" && String(first.time).trim() !== "-";
  return !timePresent;
}
...
function getLinearityKvFromTable1(doc: any): any {
  ...
  return first.kv ?? first.kV ?? first.kvp ?? first.kVp ?? first.KVP ?? first.Kvp;
}
...
const MainTestTableForInventionalRadiology: React.FC<MainTestTableProps> = ({ testData }) => {
  ...
  if (linearityDoc?.table2 && Array.isArray(linearityDoc.table2)) {
    const hasTimer = inferHasTimerFromLinearityDoc(linearityDoc);
    ...
    const validRows = linearityDoc.table2.filter((row: any) => {
      const v = appliedLabel(row);
      return v != null && String(v).trim() !== "";
    });

    if (validRows.length > 0) {
      const tolerance = linearityDoc.tolerance || "0.1";
      const toleranceOperator = linearityDoc.toleranceOperator || "<=";
      ...
      let colValue = linearityDoc.col || linearityDoc.coefficient || linearityDoc.colValue;
      ...
      let isPass = linearityDoc.remarks === "Pass" || linearityDoc.remarks === "PASS";
      if (!isPass && col !== "-") {
      ...
      const tableLevelKv =
        ...
        linearityDoc?.setKv ??
        linearityDoc?.setKV ??
        getLinearityKvFromTable1(linearityDoc);
  ...
  if (outputConsistencyRoot?.outputRows && Array.isArray(outputConsistencyRoot.outputRows)) {
    ...
    if (validRows.length > 0) {
      const tolRaw = outputConsistencyRoot.tolerance;
      ...
      const tableLevelKv =
        outputConsistencyRoot?.kv ??
        outputConsistencyRoot?.kV ??
        outputConsistencyRoot?.setKV ??
        outputConsistencyRoot?.setKv;
      const tableLevelMa =
        outputConsistencyRoot?.mA ??
        outputConsistencyRoot?.ma ??
        outputConsistencyRoot?.mas ??
        outputConsistencyRoot?.tubeCurrent ??
        outputConsistencyRoot?.setMA ??
        outputConsistencyRoot?.setMa;
      ...
      const testRows = validRows.map((row: any) => {
        ...
        let isPass = row.remark === "Pass" || row.remark === "PASS" || row.remarks === "Pass" || row.remarks === "PASS";
  ...
  if (leakParent && Array.isArray(leakMeasurements) && leakMeasurements.length > 0) {
    ...
    if (validRows.length > 0) {
      ...
      const toleranceUnit =
        validRows[0]?.unit || leakParent.leakageMeasurements?.[0]?.unit || "mGy in one hour";
  ...
  if (testData.radiationProtectionSurvey?.locations && Array.isArray(testData.radiationProtectionSurvey.locations)) {
    ...
    if (validRows.length > 0) {
      const testRows = validRows.map((loc: any) => {
        const mRPerWeek =
          loc.mRPerWeek != null && String(loc.mRPerWeek).trim() !== ""
            ? String(loc.mRPerWeek)
            : loc.mRPerHr != null && String(loc.mRPerHr).trim() !== ""
              ? String(loc.mRPerHr)
              : "-";
        ...
        const isPass =
          loc.result === "PASS" ||
          loc.result === "Pass" ||
          (mRPerWeek !== "-" && !isNaN(parseFloat(mRPerWeek)) && parseFloat(mRPerWeek) <= limit);
        return {
