...
const compareByToleranceOp = (result: number, tol: number, op: string): boolean => {
  const normalized = toAllowedToleranceOp(op);
  switch (normalized) {
    case '<':
      return result < tol;
    case '>':
...
export default function RadiationLeakageLevelFromXRay({ serviceId, testId: propTestId, tubeId, onRefresh, csvData }: Props) {
  ...
  useEffect(() => {
    if (csvData && csvData.length > 0) {
      ...
      if (tolOp) setToleranceOperator(toAllowedToleranceOp(tolOp));
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      toleranceValue,
      toleranceOperator: toAllowedToleranceOp(toleranceOperator),
      toleranceTime,
      remark: finalRemark,
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        <div className="flex flex-wrap items-center gap-2 max-w-3xl">
          ...
          <select
            ...
            onChange={(e) => setToleranceOperator(toAllowedToleranceOp(e.target.value))}
            disabled={isViewMode}
