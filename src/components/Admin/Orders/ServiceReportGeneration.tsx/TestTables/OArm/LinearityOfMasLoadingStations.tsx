...
const LinearityOfMasLoadingStationsForOArm: React.FC<Props> = ({ serviceId, testId: propTestId, onRefresh }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!currentTestId) {
        try {
          const existing = await getLinearityOfMasLoadingStationByServiceIdForOArm(serviceId);
          if (existing?._id) {
      ...
      if (currentTestId) {
        // Update existing
        result = await updateLinearityOfMasLoadingStationForOArm(currentTestId, payload);
        toast.success('Updated successfully!');
      } else {
        // Create new
        result = await createLinearityOfMasLoadingStationForOArm(serviceId, payload);
