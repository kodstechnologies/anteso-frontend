// src/components/TestTables/HighContrastResolutionForOArm.tsx
import React, { useState, useEffect, useMemo } from 'react';
import { Loader2, Edit3, Save } from 'lucide-react';
import toast from 'react-hot-toast';
import {
  createHighContrastResolutionForOArm,
  getHighContrastResolutionForOArm,
  getHighContrastResolutionByServiceIdForOArm,
  updateHighContrastResolutionForOArm,
} from "../../../../../../api";
...
const HighContrastResolutionForOArm: React.FC<Props> = ({
  ...
  useEffect(() => {
    const loadTest = async () => {
      ...
      try {
        ...
        if (propTestId) {
          data = await getHighContrastResolutionForOArm(propTestId);
        } else {
          data = await getHighContrastResolutionByServiceIdForOArm(serviceId);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      if (testId) {
        await updateHighContrastResolutionForOArm(testId, payload);
        toast.success("Updated successfully!");
      } else {
        const res = await createHighContrastResolutionForOArm(serviceId, payload);
...
export default HighContrastResolutionForOArm;
