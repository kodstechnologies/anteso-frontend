// src/components/TestTables/LowContrastResolutionForOArm.tsx
import React, { useState, useEffect, useMemo } from 'react';
import { Loader2, Edit3, Save } from 'lucide-react';
import toast from 'react-hot-toast';
import {
  createLowContrastResolutionForOArm,
  getLowContrastResolutionForOArm,
  getLowContrastResolutionByServiceIdForOArm,
  updateLowContrastResolutionForOArm,
} from "../../../../../../api";
...
const LowContrastResolutionForOArm: React.FC<Props> = ({
  ...
  useEffect(() => {
    const loadTest = async () => {
      ...
      try {
        ...
        if (propTestId) {
          data = await getLowContrastResolutionForOArm(propTestId);
        } else {
          data = await getLowContrastResolutionByServiceIdForOArm(serviceId);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      if (testId) {
        await updateLowContrastResolutionForOArm(testId, payload);
        toast.success("Updated successfully!");
      } else {
        const res = await createLowContrastResolutionForOArm(serviceId, payload);
...
export default LowContrastResolutionForOArm;
