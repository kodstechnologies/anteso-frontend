...
export default function RadiationLeakageLevel({ serviceId, testId: propTestId, onRefresh }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!currentTestId) {
        try {
          const existing = await getRadiationLeakageLevelByServiceIdForMammography(serviceId);
      ...
      if (currentTestId) {
        // Update existing
        result = await updateRadiationLeakageLevelForMammography(currentTestId, payload);
        toast.success('Updated successfully!');
      } else {
        // Create new
        result = await addRadiationLeakageLevelForMammography(serviceId, payload);
