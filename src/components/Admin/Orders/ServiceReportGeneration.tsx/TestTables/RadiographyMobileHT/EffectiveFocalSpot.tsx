...
import {
  ...
  updateEffectiveFocalSpotForRadiographyMobileHT,
} from '../../../../../../api';
...
const EffectiveFocalSpot: React.FC<Props> = ({ serviceId, testId: propTestId, onTestSaved }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let result;
      if (testId) {
        result = await updateEffectiveFocalSpotForRadiographyMobileHT(testId, payload);
      } else {
        result = await addEffectiveFocalSpotForRadiographyMobileHT(serviceId, payload);
        if (result?.data?._id) {
