...
import {
  addLinearityOfMasLoadingStationsForRadiographyMobileHT,
  getLinearityOfMasLoadingStationsByServiceIdForRadiographyMobileHT,
  updateLinearityOfMasLoadingStationsForRadiographyMobileHT,
} from '../../../../../../api';
...
const LinearityOfMasLoading: React.FC<Props> = ({ serviceId, testId: propTestId, onRefresh }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!currentTestId) {
        try {
          const existing = await getLinearityOfMasLoadingStationsByServiceIdForRadiographyMobileHT(serviceId);
      ...
      if (currentTestId) {
        // Update existing
        result = await updateLinearityOfMasLoadingStationsForRadiographyMobileHT(currentTestId, payload);
        toast.success('Updated successfully!');
      } else {
        // Create new
        result = await addLinearityOfMasLoadingStationsForRadiographyMobileHT(serviceId, payload);
  ...
  const processedTable2 = useMemo(() => {
    ...
    const colNum = hasData && xMax !== '—' && xMin !== '—' && (parseFloat(xMax) + parseFloat(xMin)) > 0
      ...
      : null;
    const col = colNum !== null ? colNum.toFixed(3) : '—';
    ...
    if (colNum !== null) {
      switch (toleranceOperator) {
        ...
        case '>=':
          ...
          break;
        case '=':
          pass = Math.abs(colNum - tol) < 0.0001; // Allow small floating point differences
          break;
        default:
          pass = colNum <= tol;
      }
    ...
    const remarks = colNum !== null ? (pass ? 'Pass' : 'Fail') : '—';
