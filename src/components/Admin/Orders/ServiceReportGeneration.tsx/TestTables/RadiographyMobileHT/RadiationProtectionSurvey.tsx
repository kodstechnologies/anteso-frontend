...
const RadiationProtectionSurvey: React.FC<Props> = ({ serviceId }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      let res;
      if (testId) {
        res = await updateRadiationProtectionSurveyForRadiographyMobileHT(testId, payload);
        toast.success("Updated successfully");
      } else {
        res = await addRadiationProtectionSurveyForRadiographyMobileHT(serviceId, payload);
