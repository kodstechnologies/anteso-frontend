...
import {
  addCongruenceForRadiographyFixed,
...
const CongruenceOfRadiation: React.FC<Props> = ({ serviceId, testId: propTestId, onTestSaved }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      let res;
      if (testId) {
        res = await updateCongruenceForRadiographyFixed(testId, payload);
        toast.success('Updated successfully!');
      } else {
        res = await addCongruenceForRadiographyFixed(serviceId, payload);
