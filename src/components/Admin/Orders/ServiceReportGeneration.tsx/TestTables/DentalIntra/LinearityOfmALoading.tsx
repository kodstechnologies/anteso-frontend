...
import {
  addLinearityOfMaLoadingForDentalIntra,
...
const LinearityOfMaLoading: React.FC<Props> = ({ serviceId, testId: propTestId, onTestSaved, csvData }) => {
      ...
      if (rowIndices.length > 0) {
        ...
        setMeasHeaders(prev => {
          const base = (csvMeasLabels
            ...
            : prev).slice(0, Math.max(maxMeas, 3));
          while (base.length < Math.max(maxMeas, 3)) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!currentTestId) {
        try {
          const existing = await getLinearityOfMaLoadingByServiceIdForDentalIntra(serviceId);
  ...
  return (
    <div className="p-6 max-w-full">
      ...
      {/* Table 2: mA + Output (mGy) with horizontal scroll and sticky mA column */}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {processedTable2.map((p, rowIdx) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-center border-r font-medium bg-yellow-50 min-w-[80px]">
                    {p.xMax}
                  </td>
                  <td className="px-4 py-2 text-center border-r font-medium bg-yellow-50 min-w-[80px]">
                    {p.xMin}
