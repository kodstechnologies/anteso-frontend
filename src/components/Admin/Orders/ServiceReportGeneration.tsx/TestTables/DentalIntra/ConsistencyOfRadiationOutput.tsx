...
import {
  addConsistencyOfRadiationOutputForDentalIntra,
...
const ConsistencyOfRadiationOutput: React.FC<Props> = ({
        ...
        if (!isNaN(tol) && !isNaN(cov)) {
          switch (toleranceOperator) {
            ...
            case '>=':
              ...
              break;
            case '=':
              remarks = Math.abs(cov - tol) < 0.0001 ? 'Pass' : 'Fail';
              break;
            case '<=':
            default:
              remarks = cov <= tol ? 'Pass' : 'Fail';
  ...
  useEffect(() => {
    const loadTest = async () => {
      ...
      try {
        ...
        if (propTestId) {
          const res = await getConsistencyOfRadiationOutputByTestIdForDentalIntra(propTestId);
          data = res?.data;
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      let result;
      if (testId) {
        result = await updateConsistencyOfRadiationOutputForDentalIntra(testId, payload);
        toast.success('Updated successfully!');
      } else {
        result = await addConsistencyOfRadiationOutputForDentalIntra(serviceId, payload);
