...
import {
  ...
  updateAccuracyOfOperatingPotentialAndTimeForDentalHandHeld,
} from "../../../../../../api";
...
const AccuracyOfOperatingPotentialAndTime: React.FC<Props> = ({ 
  ...
  const calculateRow = (row: RowData): RowData => {
    ...
    const validKvps = stations
      .map(s => parseFloat(s?.kvp || s?.kvp || "0") || 0)
      .filter(v => v > 0);
    const validTimes = stations
      .map(s => parseFloat(s?.time || s?.time || "0") || 0)
      .filter(v => v > 0);
  ...
  useEffect(() => {
    ...
    const loadTest = async () => {
      setIsLoading(true);
      try {
        const data = await getAccuracyOfOperatingPotentialAndTimeByServiceIdForDentalHandHeld(serviceId);
        if (data?.data) {
  ...
  const saveTest = async () => {
    ...
    try {
      ...
      let result;
      if (testId) {
        result = await updateAccuracyOfOperatingPotentialAndTimeForDentalHandHeld(testId, payload);
      } else {
        result = await addAccuracyOfOperatingPotentialAndTimeForDentalHandHeld(serviceId, payload);
        if (result?.data?._id) {
