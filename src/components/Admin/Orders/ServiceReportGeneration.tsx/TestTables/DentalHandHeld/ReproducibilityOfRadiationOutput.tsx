...
import {
  addReproducibilityOfRadiationOutputForDentalHandHeld,
  getReproducibilityOfRadiationOutputByServiceIdForDentalHandHeld,
  updateReproducibilityOfRadiationOutputForDentalHandHeld,
} from "../../../../../../api";
...
const ReproducibilityOfRadiationOutput: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    ...
    const loadTest = async () => {
      setIsLoading(true);
      try {
        const data = await getReproducibilityOfRadiationOutputByServiceIdForDentalHandHeld(serviceId);
        if (data?.data) {
