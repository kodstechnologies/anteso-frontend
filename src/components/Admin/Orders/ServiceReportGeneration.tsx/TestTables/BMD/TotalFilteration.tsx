...
import {
    addTotalFiltrationForBMD,
    getTotalFiltrationByTestIdForBMD,
...
const TotalFilterationForInventionalRadiology: React.FC<TotalFilterationForInventionalRadiologyProps> = ({
    ...
    const saveTest = async () => {
        ...
        try {
            ...
            if (!currentTestId) {
                try {
                    const existing = await getTotalFiltrationByServiceIdForBMD(serviceId);
            ...
            if (currentTestId) {
                // Update existing
                result = await updateTotalFiltrationForBMD(currentTestId, payload);
                toast.success("Updated successfully");
            } else {
                // Create new
                result = await addTotalFiltrationForBMD(serviceId, payload);
