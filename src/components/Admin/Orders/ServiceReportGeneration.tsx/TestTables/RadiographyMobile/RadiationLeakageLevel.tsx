...
export default function TubeHousingLeakage({ serviceId, testId: propTestId, onRefresh }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (currentTestId) {
        result = await updateRadiationLeakageLevelForRadiographyMobile(currentTestId, payload);
        toast.success('Updated successfully!');
      } else {
        result = await addRadiationLeakageLevelForRadiographyMobile(serviceId, payload);
