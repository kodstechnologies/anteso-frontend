...
import {
  addRadiationLeakageLevelForRadiographyMobile,
  getRadiationLeakageLevelByServiceIdForRadiographyMobile,
  updateRadiationLeakageLevelForRadiographyMobile,
} from '../../../../../../api';
...
interface Props {
  ...
  initialData?: {
    ...
    workload?: string;
    tolerance?: { value: string; operator: string };
    leakageMeasurements?: { location: string; left: string; right: string; front: string; back: string; top: string }[];
  } | null;
}
...
export default function RadiationLeakageLevel({ serviceId, testId: propTestId, onRefresh, refreshKey, initialData }: Props) {
  ...
  // Start with Tube only (Collimator can be added dynamically)
  ...
  // Process each row: calculate max, result (mR/h), mGy/h, and per-row remark
  ...
  const maxExposureLevel = Math.max(...processedLeakage.map(row => parseFloat(row.max) || 0)).toFixed(2);
  ...
  const updateLeakage = (index: number, field: keyof LeakageRow, value: string) => {
    setLeakageRows(prev => prev.map((r, i) => (i === index ? { ...r, [field]: value } : r)));
  };
  ...
  const removeLeakageRow = (index: number) => {
    if (leakageRows[index].location === 'Tube') {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (err.response?.status !== 404) {
          toast.error('Failed to load Radiation Leakage Level data');
        }
        setIsEditing(true);
      } finally {
        setIsLoading(false);
  ...
  // Pre-fill from Excel initialData when it arrives
  useEffect(() => {
    ...
    if (initialData.leakageMeasurements?.length) {
      // Always ensure Tube is first
      const sorted = [...initialData.leakageMeasurements].sort((a, b) =>
        a.location === 'Tube' ? -1 : b.location === 'Tube' ? 1 : 0
      );
      setLeakageRows(sorted.map(m => ({
        location: m.location || '',
        left: String(m.left ?? ''),
        right: String(m.right ?? ''),
        front: String(m.front ?? ''),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!currentTestId) {
        try {
          const existing = await getRadiationLeakageLevelByServiceIdForRadiographyMobile(serviceId);
          if (existing?.data?._id) { currentTestId = existing.data._id; setTestId(currentTestId); }
        } catch (_) { }
      }

      if (currentTestId) {
        await updateRadiationLeakageLevelForRadiographyMobile(currentTestId, payload);
        toast.success('Updated successfully!');
      } else {
        const result = await addRadiationLeakageLevelForRadiographyMobile(serviceId, payload);
  ...
  const toggleEdit = () => { if (!hasSaved) return; setIsEditing(true); };
      ...
      <div className="flex justify-end mt-8">
        <button
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            isViewMode ? toggleEdit() : handleSave();
          }}
        ...
        >
          {isSaving ? (
            ...
            <><ButtonIcon className="w-5 h-5" />{buttonText} Radiation Leakage Level</>
          )}
        </button>
      </div>
    </div>
