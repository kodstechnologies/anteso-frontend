...
const RadiationProtectionSurvey: React.FC<Props> = ({ serviceId, csvData, initialSurveyDate }) => {
  ...
  // CSV Data Injection — do not import Survey Date from Excel (use QA test date, else today)
  useEffect(() => {
    if (csvData && csvData.length > 0) {
      const applyQaSurveyDate = async () => {
        if (!serviceId) {
