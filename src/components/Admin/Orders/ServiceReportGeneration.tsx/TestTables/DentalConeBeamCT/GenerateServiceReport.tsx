...
const DentalConeBeamCT: React.FC<{ serviceId: string; qaTestDate?: string | null; csvFileUrl?: string | null }> = ({ serviceId, qaTestDate, csvFileUrl }) => {
    ...
    const getUnsavedTestNames = async (): Promise<string[]> => {
        const checks: { name: string; check: () => Promise<boolean> }[] = [
            { name: "Accuracy Of Operating Potential", check: async () => { try { return isSaved(await getAccuracyOfOperatingPotentialByServiceIdForCBCT(serviceId)); } catch { return false; } } },
            { name: "Consistency Of Radiation Output", check: async () => { try { return isSaved(await getConsistencyOfRadiationOutputByServiceIdForCBCT(serviceId)); } catch { return false; } } },
            { name: "Radiation Leakage Level", check: async () => { try { return isSaved(await getRadiationLeakageLevelByServiceIdForCBCT(serviceId)); } catch { return false; } } },
            { name: "Details Of Radiation Protection", check: async () => { try { return isSaved(await getRadiationProtectionSurveyByServiceIdForCBCT(serviceId)); } catch { return false; } } },
        ];
        if (hasTimer === true) {
            checks.push({ name: "Accuracy Of Irradiation Time", check: async () => { try { return isSaved(await getAccuracyOfIrradiationTimeByServiceIdForCBCT(serviceId)); } catch { return false; } } });
            checks.push({ name: "Linearity Of mA Loading", check: async () => { try { return isSaved(await getLinearityOfMaLoadingByServiceIdForCBCT(serviceId)); } catch { return false; } } });
        } else if (hasTimer === false) {
            checks.push({ name: "Linearity Of mAs Loading", check: async () => { try { return isSaved(await getLinearityOfMaLoadingByServiceIdForCBCT(serviceId)); } catch { return false; } } });
        }
            ...
            <div className="my-10 flex justify-end gap-6">
                ...
                <button
                    onClick={async () => {
                        ...
                        navigate(`/admin/orders/view-service-report-dental-cone-beam-ct?serviceId=${serviceId}`);
                    }}
