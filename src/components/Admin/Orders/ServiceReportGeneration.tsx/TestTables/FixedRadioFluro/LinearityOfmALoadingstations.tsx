...
const LinearityOfMaLoading: React.FC<Props> = ({ serviceId, testId: propTestId, onRefresh, refreshKey, initialData }) => {
  ...
  const processedTable2 = useMemo(() => {
    ...
    const rowsWithX = table2Rows.map(row => {
      ...
      let x = null;
      if (avg !== '—' && mAs && mAs > 0) {
        x = parseFloat(avg) / mAs;
      } else if (avg !== '—' && !isNaN(ma) && ma > 0) {
        x = parseFloat(avg) / ma;
      }
  ...
  useEffect(() => {
    if (initialData && refreshKey !== undefined) {
      console.log('LinearityOfmALoading: Loading CSV data', initialData);
      if (initialData.table1) {
        ...
        if (initialData.table1.time) setTable1Row(prev => ({ ...prev, time: String(initialData.table1.time) }));
      }
