...
const LinearityOfMasLoading: React.FC<Props> = ({
  ...
  const processedTable2 = useMemo(() => {
    ...
    if (hasData && col !== '—') {
      ...
      switch (toleranceOperator) {
        case '<': pass = colVal < tol; break;
        case '>': pass = colVal > tol; break;
        case '<=': pass = colVal <= tol; break;
        case '>=': pass = colVal >= tol; break;
        case '=': pass = Math.abs(colVal - tol) < 0.0001; break;
        default: pass = colVal <= tol;
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (currentTestId) {
        await updateLinearityOfMasLoadingForFixedRadioFluro(currentTestId, payload);
        toast.success('Updated successfully!');
      } else {
        const result = await addLinearityOfMasLoadingForFixedRadioFluro(serviceId, payload);
