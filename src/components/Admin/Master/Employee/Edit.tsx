...
const EditEngineer = () => {
  ...
  const handleSubmit = async (
    values: FormValues,
    helpers: FormikHelpers<FormValues>   // ← changed from _helpers to helpers
  ) => {
    try {
      const technicianType: RoleValue = normalizeRole(values.role as string);
      const status: StatusValue = normalizeStatus(values.status as string);
      ...
      navigate('/admin/employee');
        ...
        // ── Fix: use err.response.status (not statusCode) ──
        if (err?.response?.status === 409) {
          // Now we can safely read the message
          errorMessage = err.response.data?.message
            || "This phone number is already in use.";

          // Show error under the phone field
          if (errorMessage.toLowerCase().includes("phone")) {
            helpers.setFieldError("phone", errorMessage);
          }
        }
