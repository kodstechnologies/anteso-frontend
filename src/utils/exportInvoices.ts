...
export const mapInvoiceToExportRow = (invoice: Record<string, any>): InvoiceExportRow => ({
    ...
    createdAt: formatCreatedAtDisplay(invoice.createdAt),
    grandtotal: invoice.grandtotal ? `₹${invoice.grandtotal.toLocaleString()}` : '₹0',
...
const getExportFileName = (extension: string) => {
    ...
    return `invoices-export-${dateStamp}.${extension}`;
};
...
const getInvoiceCardFields = (row: InvoiceExportRow): CardField[] => [
    ...
    { label: 'Total Amount', value: row.grandtotal },
];
